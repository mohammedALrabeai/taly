...
class SettingsService extends GetxService {
  ...
  Future<SettingsService> init() async {
    ...
    setting.value = await _settingsRepo.get2();
    loopLoca();
    loopCity();
    await getAddress();
    return this;
  ...
  getCurrent() async {
    MyLocation2 loc = MyLocation2();
    try {
      myLocation = await loc.getCurrent();
    } catch (e) {
      log(e.toString());
    }
  }
  ...
  void getCities() {
    ...
    // .where('created_by', isEqualTo: FirebaseAuth.instance.currentUser!.uid)
        .get()
        .then((value) {
      cities.value = value.docs.map((e) {
        log(e.data().toString());
        var r=  City.fromJson(e.data());
        r.id=e.id;
        return r;
      }).toList();
      cities.refresh();
