...
class LaravelApiClient extends GetxService with ApiClient {
  ...
  Future<List<EService>> getRecommendedEServices() async {
    ...
    var _queryParameters = {
      'only': 'id;name;price;distance;discount_price;price_unit;has_media;media;total_reviews;rate;provider',
      'limit': '6',
    };
  ...
  Future<List<EService>> getAllEServicesWithPagination(String categoryId, int page) async {
    final _address = Get.find<SettingsService>().address.value;
    var _queryParameters = {
      'with': 'eProvider;distance;eProvider.addresses;categories',
  ...
  Future<Setting> getSettings() async {
    ...
    if (response.data['success'] == true) {
      ...
      Get.log(response.data['message'].toString());
      throw new Exception(response.data['message']);
    }
  }
  ...
  getImage(Shop value) async {
    var pat=value.logo.split("/").last;
    // Direc
    Directory path=await pp.getApplicationDocumentsDirectory();
  var jj=  join(path.path,pat);
    var res= await _httpClient.downloadUri(Uri.parse(value.logo),jj);
  // var d=  Helper.getFilesInDirectory(jj);
    return res;
  }
}
