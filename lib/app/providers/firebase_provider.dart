  ...
  Future<void> sendCodeToPhone2(Function(bool) onauth) async {
    ...
    _verifyFailed(fba.FirebaseAuthException e) {
      ...
      if(e.toString().contains("too-many-requests")) {
        Get.showSnackbar(Ui.ErrorSnackBar(title: "محاولات كثيرة جدا",message: "لقد حظرنا جميع الطلبات الواردة من هذا الجهاز بسبب نشاط غير عادي. حاول مرة أخرى في وقت لاحق."));
      } else if(e.toString().contains("timeout")) {
        Get.showSnackbar(Ui.ErrorSnackBar(title: "تاكد من اتصالك بالإنترنت",message: "ثم حاول مرة اخرى"));
      } else if(e.toString().contains("missing-client")) {
        Get.showSnackbar(Ui.ErrorSnackBar(title: "عذراً",message:"اعد ادخال رقم هاتفك من جديد"));
      } else {
        Get.showSnackbar(Ui.ErrorSnackBar(title: "عذراً",message: "يرجى المحاولة لاحقاً"));
      }
      throw Exception(e.message);
    }
