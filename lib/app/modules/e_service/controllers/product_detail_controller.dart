...
class ProductDetailController extends GetxController {
  ...
  void setPrts(Product category) {
    ...
    if(category.parts!=null &&category.parts.isNotEmpty) {
      ...
      selectCut.value =productDetails.value.cuts==null ||productDetails.value.cuts.isEmpty? "0": productDetails.value.cuts?.first ?? "";
    }else{
...
MyLocation2 location2=MyLocation2();
  num getprice(Parts value) {
    log(selectPart.value.toJson().toString()+" :: "+cities.length.toString());
    num res=selectPart.value.price;
    if(myLocation!=null){
      var locs=cities.where((p0) {
        var dest=location2.distanceBetween(myLocation.latitude, myLocation.longitude, p0.latlong.latitude, p0.latlong.longitude);
         log("d:"+dest.toString());
       return dest<=p0.area;
      }).toList();
log("lllll= "+locs.length.toString());
      if(locs!=null&&locs.isNotEmpty)
      if(selectPart.value.c_price!=null&&selectPart.value.c_price.isNotEmpty){
        selectPart.value.c_price.forEach((element) {
       var c=  locs.firstWhereOrNull((l) => l.id==element.place_id && element.price!=null);
         if(c!=null){
           res=element.price;
         }
        });
